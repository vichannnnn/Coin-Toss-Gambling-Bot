...
c.execute('''CREATE TABLE IF NOT EXISTS server (`serverID` INT PRIMARY KEY, `embed` TEXT) ''')
...
async def colourChange(ctx, colour):
    ...
    conn.commit()
    return await successEmbedTemplate(ctx,
                                      f"Embed colour successfully set to **{colour}** for **{ctx.message.guild}**.",
                                      ctx.message.author)
